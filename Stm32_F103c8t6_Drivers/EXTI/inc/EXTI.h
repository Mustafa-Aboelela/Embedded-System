...
typedef struct
{
	EXTI_GPIO_mapping_t EXTI_Pin ;           //this parameter must be based on @ref EXTI_define
	uint8_t  trigger_status ;                //this parameter must be based on @ref EXTI_trigger_define
	uint8_t IRQ_EN ;                         //the parameter must be based on @ref EXTI_IRQ_define
	void (*P_IRQ_CallBack);                  //set the C function (ISR) once the IRQ happen
